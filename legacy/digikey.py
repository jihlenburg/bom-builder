...
class DigiKeyClient:
    ...
    def document_links(
        ...
        """Return datasheet and product-detail URLs for one Digi-Key orderable.

        Parameters
        ----------
        product_number:
            Digi-Key orderable number accepted by Product Information V4.

        Returns
        -------
        tuple[str | None, str | None]
            ``(datasheet_url, product_url)`` extracted from the normalized
            product-details payload.
        """
        payload, _header_mode = self.product_details(product_number)
        product = payload.get("Product")
        product_data = product if isinstance(product, dict) else {}
        return (
            _optional_str(product_data, "DatasheetUrl"),
            _optional_str(product_data, "ProductUrl"),
        )
...
def _parse_pricing_result(
    ...
    *,
    ...
    return DigiKeyPricingResult(
        ...
        product_url=_optional_str(payload, "ProductUrl"),
...
def price_part_via_digikey(
    ...
    *,
    ...
    include_documents:
        Whether to make the cache-backed product-details request needed to
        capture Digi-Key's manufacturer datasheet link.
    ...
    for query in attempted:
        ...
        if include_documents:
            try:
                datasheet_url, detail_product_url = client.document_links(
                    distributor_pn
                )
                product_url = detail_product_url or product_url
            except Exception as exc:
                log.debug(
                    "Digi-Key document discovery failed for %s: %s",
                    distributor_pn,
                    type(exc).__name__,
                )
