...
SUPPORTED_DISTRIBUTORS = ("mouser", "digikey", "ti", "nxp")
...
def price_parts(
    ...
    *,
    ...
    selected_providers = {
        ...
        for provider in getattr(args, "providers", SUPPORTED_DISTRIBUTORS)
    }

    console.print("\n[heading]Looking up distributor prices...[/heading]")
    with ExitStack() as stack:
        mouser_client = (
            stack.enter_context(MouserClient(
                api_key=getattr(args, "mouser_api_key", ""),
                **cache_kw,
            ))
        ...
        ai_resolver = (
            ...
            if args.ai_resolve and mouser_client is not None
            else None
        )
        digikey_client = (
            stack.enter_context(DigiKeyClient(**cache_kw))
            if "digikey" in selected_providers and digikey_is_configured()
            else None
        )
        ti_client = (
            stack.enter_context(TIClient(**cache_kw))
            if "ti" in selected_providers and ti_is_configured()
            else None
        )
        nxp_client = (
            stack.enter_context(NXPClient(**cache_kw))
            if "nxp" in selected_providers and nxp_is_available()
            else None
        )
        ...
        return _price_parts_across_distributors(
            ...
            include_documents=getattr(args, "include_documents", False),
        )
...
def _price_single_part(
    agg: AggregatedPart,
    mouser_client: MouserClient | None,
    *,
    ...
    mouser_client:
        Optional active Mouser API client. When omitted, the part starts as an
        unresolved pricing record and the other selected providers may resolve
        it independently.
    digikey_client:
    ...
    include_documents:
        Whether provider adapters may make cache-backed detail requests to
        attach datasheet and product URLs.
...
def _mouser_request_count(mouser_client: MouserClient | None) -> int:
    """Return the tracked live Mouser request count for pacing decisions.

    A missing client represents a run that intentionally excluded Mouser and
    therefore contributes zero paced requests.
    """
