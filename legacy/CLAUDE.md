...
BOM Builder is a machine-first Python CLI for building and pricing electrical Bills of Materials (eBOMs) across Mouser, Digi-Key, TI, and NXP. It reads design JSON files, aggregates parts, performs provider-aware resolution and price comparison, and generates versioned JSON plus optional CSV/Excel buyer artifacts.
...
- Canonical CLI parsing, machine envelopes, maintenance commands, and command
  dispatch stay in `bom_builder_cli.py`; their Pydantic contracts stay in
  `cli_models.py`.
- Shared pricing orchestration stays in `main.py` so the canonical CLI and TUI
  reuse the same provider-selection and offer-comparison engine.
