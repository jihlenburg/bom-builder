...
def convert_offer_currency(
    ...
    converted_legs = []
    for leg in offer.purchase_legs:
        leg_src = _normalized_currency(leg.currency)
        if not leg_src or leg_src == dst:
            converted_legs.append(leg)
            continue
        leg_quote = rate_provider.quote(leg_src, dst)
        converted_legs.append(
            leg.model_copy(
                update={
                    "currency": dst,
                    "unit_price": round(leg.unit_price * leg_quote.rate, 6),
                    "extended_price": round(leg.extended_price * leg_quote.rate, 2),
                }
            )
        )
