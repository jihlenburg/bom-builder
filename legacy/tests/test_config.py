...
from config import DATA_DIR, install_console_trace, resolve_trace_path, setup_logging


def test_runtime_data_directory_contains_yaml_knowledge_bases():
    """Source and installed runtimes must ship both resolver data files."""
    assert (DATA_DIR / "manufacturers.yaml").is_file()
    assert (DATA_DIR / "packages.yaml").is_file()
