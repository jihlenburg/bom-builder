...
PROJECT_VERSION = "2.0.0.0"
DEFAULT_ATTRITION = 0.0


def _resolve_data_dir() -> Path:
    """Return the directory containing packaged YAML knowledge bases.

    Editable/source checkouts keep ``manufacturers.yaml`` and
    ``packages.yaml`` beside the top-level modules. Built wheels install those
    files under the interpreter data prefix in ``share/bom-builder``. Checking
    both locations keeps repository editing convenient while making normal
    package installs complete.
    """
    source_dir = Path(__file__).parent
    if (
        (source_dir / "manufacturers.yaml").exists()
        and (source_dir / "packages.yaml").exists()
    ):
        return source_dir
    return Path(sysconfig.get_path("data")) / "share" / "bom-builder"
