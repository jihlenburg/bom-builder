...
class ResolutionStore:
    ...
    def list_records(self) -> tuple[ResolutionRecord, ...]:
        """Return every valid saved resolution in deterministic key order."""
        ...
        for key in sorted(self._data):
            ...
            if not isinstance(raw, dict):
                continue
            try:
                records.append(ResolutionRecord(**raw))
            except TypeError:
                continue
        return tuple(records)

    def remove(self, manufacturer: str, part_number: str) -> bool:
        """Remove one saved resolution and return whether it existed."""
        key = _resolution_key(manufacturer, part_number)
